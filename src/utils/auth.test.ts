...
describe("AuthHelper (Functional)", () => {
  ...
  it("should generate a valid seed in ISO 8601 format", async () => {
    const auth = await generateAuth(login, secretKey)
  ...
  it("should generate a tranKey that matches the Placetopay algorithm", async () => {
    /**
  ...
  it("should produce different nonces on every call", async() => {
    const auth1 = await generateAuth(login, secretKey);
    const auth2 = await generateAuth(login, secretKey);
